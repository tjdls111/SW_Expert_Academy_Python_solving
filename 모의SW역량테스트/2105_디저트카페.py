...
def dfs(i, j, direction_type, dessert_cnt):
    ...
    for k in range(direction_type, tmp):
        ...
        if 0 <= ni < N and 0 <= nj < N:
            if arr[ni][nj] not in dessert_visited and cafe_visited[ni][nj] == False:  # 방문하지 않은 카페 & 먹지 않은 디저트

                cafe_visited[ni][nj] = True
                dessert_visited.add(arr[ni][nj])

                dfs(ni, nj, k, dessert_cnt + 1)

                cafe_visited[ni][nj] = False
                dessert_visited.remove(arr[ni][nj])
...
for tc in range(1, T + 1):
    ...
    for i in range(N):
        for j in range(N):
            ...
            dessert_visited.add(arr[i][j])

            dfs(i, j, 0, 1)

            cafe_visited[i][j] = False
            dessert_visited.remove(arr[i][j])
